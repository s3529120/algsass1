...
public class Point {
    ...
    private double rad2deg(double rad) {
     ...
     // Following section added by Thomas Higgins
     
    /** Early termination bubble-sort on list of points.
     * @param points List of points to sort in ascending order.
     * @param split Axis by which to sort list.
     * @return Sorted array of points.
     */
    public static Point[] sortPoints(List<Point> points,Split split){
       ...
       if(split==Split.lon){
          ...
          for(int i=0;i<points.size()-1;i++){
             for(int j=0;j<points.size()-2-i;j++){
                if(array[j+1].lat<array[j].lat){
                   swap=array[j];
                   array[j]=array[j+1];
                   array[j+1]=swap;
                   swapMade=true;
    ...
    /** Get median point from array of points.
     * @param points List of points to extract median value;
     * @return Median point.
     */
    public static int getMedian(Point[] points){
